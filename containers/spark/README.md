...
```bash
docker run --rm ghcr.io/seathegood/data-platform-containers/spark-runtime:4.1.2 --version
```
...
docker run --rm \
  -v "$PWD/app:/opt/app" \
  -w /opt/app \
  ghcr.io/seathegood/data-platform-containers/spark-runtime:4.1.2 \
...
`ICEBERG_RUNTIME_FLAVOR` controls the Spark 4.x Iceberg artifacts (e.g., `4.1_2.13`). Change it only when targeting a different Spark/Iceberg matrix and rerun the smoke tests.
